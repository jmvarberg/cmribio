# Package index

## All functions

- [`camera_dotplot()`](https://jmvarberg.github.io/cmribio/reference/camera_dotplot.md)
  : Create dot plots of cameraPR results by direction and contrast
- [`cmri_datatable()`](https://jmvarberg.github.io/cmribio/reference/cmri_datatable.md)
  : Interactive data tables
- [`create_bcb_report()`](https://jmvarberg.github.io/cmribio/reference/create_bcb_report.md)
  : Generate a standard analysis report from template in the current
  directory.
- [`demo_counts`](https://jmvarberg.github.io/cmribio/reference/demo_counts.md)
  : Example bulk RNAseq counts matrix
- [`demo_samplesheet`](https://jmvarberg.github.io/cmribio/reference/demo_samplesheet.md)
  : Example bulk RNAseq sample sheet
- [`get_genesets()`](https://jmvarberg.github.io/cmribio/reference/get_genesets.md)
  : Pull Gene Sets of Interest from msigdbr package
- [`init_proj()`](https://jmvarberg.github.io/cmribio/reference/init_proj.md)
  : Initialize Project Directory
- [`list_targets_templates()`](https://jmvarberg.github.io/cmribio/reference/list_targets_templates.md)
  : List available targets pipeline templates
- [`pathway_heatmap()`](https://jmvarberg.github.io/cmribio/reference/pathway_heatmap.md)
  : Heatmap of pathway of interest
- [`run_cameraPR()`](https://jmvarberg.github.io/cmribio/reference/run_cameraPR.md)
  : Run limma::cameraPR
- [`swap_ensmbl_for_symbols()`](https://jmvarberg.github.io/cmribio/reference/swap_ensmbl_for_symbols.md)
  : Convert ENSMBL IDs to Gene Symbols
- [`use_targets_template()`](https://jmvarberg.github.io/cmribio/reference/use_targets_template.md)
  : Use targets pipeline templates
